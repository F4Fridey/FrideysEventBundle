...
namespace FrideysEventBundle
{
    class EventCommand : ICommandHandler
    {
		...
		public string[] OnCall(ICommandSender sender, string[] args)
		{
			...
			{
				return new string[] { "\n[ Frideys Event Bundle ]\nMain commands:\nevent add <event> - Add an event to the qeue.\nevent qeue view - Prints the current event qeue.\nevent qeue clear - Clears the current event qeue.\nEvents:\nnoevent\nchaosvsntf - Needs 2 people. Half of the server is NTF, the others are Chaos. The two must fight it out.\npeanutpocalypse - Needs 2 people. Spawns 1 peanut and the rest are D class. The d class must get to the gates to get MICROs while the peanuts must kill the D class to duplicate.\ndclassbattle - Needs 4 people. Class D Battle Royal.\ndclassinvasion - Needs 4 people. 1 - 3 NTF spawn with 5000 HP and must survive for 7 minutes. The rest are D class that constantly respawn with guns.\nttt - Needs 3 people. 1 murderer that must kill everyone, 1 sherif that must kill the murderer, and the rest are innocent of which can pick up the gun to become a sherif if the sherif dies." };
			}
